...
<div x-data="transaksiPage()" x-init="fetchProduk()" class="bg-gray-50 min-h-[100dvh] p-2 lg:p-4 space-y-4">
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 h-[86dvh]">

    <div class="lg:col-span-8 bg-white p-4 rounded-xl shadow-lg h-full flex flex-col">

      <div class="mb-4 sticky top-0 bg-white z-20 pb-4 border-b border-gray-100">
        <input id="searchProduk" type="text" placeholder="Cari produk dengan nama atau kode..." autofocus
          x-model="search"
          class="w-full p-3 font-medium rounded-lg text-lg bg-gray-50 text-gray-800 border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 transition"
          @input.debounce.300="fetchProduk()"
          @keydown.enter.prevent="tambahProdukDariInput()">
      </div>

      <div class="flex-1 overflow-y-auto pr-2 custom-scrollbar">
        <div x-show="produk.length === 0" class="text-center text-gray-500 py-10">
          ...
          <template x-for="p in produk" :key="p.kode_produk">
            <div
              ...
              <div class="relative">
                ...
                <span
                  class="absolute top-2 right-2 text-xs font-bold px-3 py-1 rounded-full shadow-lg transition-all duration-200"
                  :class="{'bg-red-500 text-white': p.stok == 0, 'bg-yellow-400 text-gray-800': p.stok > 0 && p.stok <= 10, 'bg-emerald-500 text-white': p.stok > 10}"
                  x-text="p.stok > 0 ? p.stok + ' unit' : 'HABIS'">
                </span>
              </div>
              ...
              <div class="p-2">
                ...
                <p class="text-lg font-extrabold text-emerald-600 mt-1" x-text="formatRupiah(p.harga)"></p>
              </div>
            </div>
          </template>
        </div>
    ...
    <div class="lg:col-span-4 bg-white rounded-xl shadow-2xl flex flex-col h-full">
      <div class="flex items-center justify-between text-emerald-700 mb-2 pt-4 px-4 border-b pb-4">
      ...
      <div x-show="keranjang.length === 0" class="text-gray-500 text-center p-10 flex-1">
      ...
      <div class="flex flex-1 flex-col space-y-3 p-4 overflow-y-auto max-h-[40vh]">
        <template x-for="(item, index) in keranjang" :key="item.kode_produk">
          <div class="flex justify-between items-center border-b border-gray-200 pb-3 hover:bg-gray-50 -mx-4 px-4 transition duration-150">
            ...
            <div class="flex items-center gap-2">
              ...
              <p class="text-sm font-bold text-gray-900 w-16 text-right" x-text="formatRupiah(item.subtotal)"></p>
      ...
      <div class="mt-auto border-t-4 border-emerald-200 pt-3 space-y-2 p-3 bg-gray-50 rounded-b-xl shadow-inner">
        ...
        <div class="space-y-1 pt-1">
          <button @click="simpanTransaksi(true)" :disabled="keranjang.length === 0"
          ...
          <button @click="simpanTransaksi(false)" :disabled="keranjang.length === 0"
